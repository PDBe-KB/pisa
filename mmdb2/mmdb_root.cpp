...
//  bool      NewChain;
  ...
  mmcif::PLoop Loop,LoopAnis,LoopRemap;
  int          i,j,index,nATS,l;
  ERROR_CODE   RC, RC2;
    ...
    LoopRemap = CIFD->GetLoop ( CIFCAT_PDBX_CHAIN_REMAPPING ); //GDL add remapping loop- test
    if(LoopRemap) l = LoopRemap->GetLoopLength(); //GDL, lenght of remapping loop
    ...
    for (i=1;i<=nATS;i++)  {
      ...
      // ReadPDBAtom(..).
      index = nAtoms+1;  // index for the next atom in Atom array
      RC    = CheckAtomPlace ( index,Loop, LoopRemap );
      
      if (!RC)
	{
	  //atom[index-1]->GetAtomSiteCIF ( i,Loop);
	  RC = atom[index-1]->GetCIF ( i,Loop,LoopAnis);
          //atom[index-1]->GetremapCIF ( i,LoopRemap,Loop);
	  if (LoopRemap) RC2=atom[index-1]->GetremapCIF ( i,LoopRemap);
	}
  ...
  ERROR_CODE Root::CheckAtomPlace ( int index, mmcif::PLoop Loop, mmcif::PLoop LoopRemap )  {
  ...
  //GDL:Commented out label_seq_id int
  //int      seqNum,label_entity_id,RC,k,nM;
  //LabelSeqID label_seq_id;
  ChainID  chainID,label_asym_id;
  InsCode  insCode;
  pstr     F;

  //int         j,l,m,R,remap_entity_id;
  //pstr        L,A,E,G,H;
  //ChainID     orig_label_asym_id;
  ...
  //  if (!CIFGetInteger1(seqNum,Loop,CIFTAG_LABEL_SEQ_ID,k))
    ...
    CIFGetInteger1 ( label_seq_id   ,Loop,CIFTAG_LABEL_SEQ_ID   ,k ); //GDL:commented out int, changed to char
    ...
    //CIFGetString ( label_seq_id,Loop,CIFTAG_LABEL_SEQ_ID,k,
    //               sizeof(label_seq_id),pstr("") );
  ...
  ERROR_CODE Root::AllocateAtom ( int           index,
    ...
    //strcpy ( crRes->label_seq_id,label_seq_id ); //GDL:change label_seq_id to char instead of int
