...
//  bool      NewChain;
  ...
  ERROR_CODE Root::ReadCIFAtom ( mmcif::PData CIFD )  {
    mmcif::PLoop Loop,LoopAnis,LoopRemap; //GDL:added LoopRemap variable
    int          i,j,index,nATS,l; //GDL:added j and l indexes
    ERROR_CODE   RC, RC2; //GDL:added RC2
    ...
    for (i=1;i<=nATS;i++)  {
      ...
      // ReadPDBAtom(..).
      index = nAtoms+1;  // index for the next atom in Atom array
      RC    = CheckAtomPlace ( index,Loop,LoopRemap ); //GDL:added LoopRemap
      if (!RC)
        {
          RC = atom[index-1]->GetCIF ( i,Loop,LoopAnis);
          
          if (LoopRemap) RC2=atom[index-1]->GetremapCIF ( i,LoopRemap); //GDL:added LoopRemap
        }
