...
namespace ccp4srs  {
  ...
  int             natom1, natom2,ncontvW;
  mmdb::PPAtom    atom1, atom2,atomtest;
  mmdb::PContact  contvW;
    ...
    //GDL test:start
    //MMDB->SeekContacts (atom1,natom1,atom2,natom2,0.0,5.0,0,contvW,ncontvW,3,NULL,0,0 );
    /*
    for (i=0;i<nres1;i++)
      {
	Res1[i]->GetAtomTable ( atom1,natom1 );
	//for (i=0;i<natom1;i++) cout << atom1[i]->element<<"\n";
	}
    for (i=0;i<nres2;i++)
      {
	 Res2[i]->GetAtomTable(atom2,natom2);
        //for (i=0;i<natom1;i++) cout << atom1[i]->element<<"\n";                       
	}
    for (i=0;i<nres1;i++) cout << "res-loop-1"<<"\t"<<Res1[i]->GetResName()<<"\n";
    for (i=0;i<nres2;i++) cout << "res-loop-2"<<"\t"<<Res2[i]->GetResName()<<"\n";
    */
    //GDL test:end
    ...
    for (i=0;i<nContacts;i++)  {
      ...
      //GDL check salt-bridges
      //if (isSBridge)  {
      //cout << "Contact-SB"<<"\t"<< D->GetResName() <<"\t"<<D->GetSeqNum ()<<"\t"<<D->name<<"\t"<< A->GetResName() <<"\t"<<A->GetSeqNum()<<"\t"<<A->name<<"\t"<<Contact[i].dist<< "\n";
      //}
      ...
      //GDL:check H-Bonds list
      //if (isHBond && !isSBridge){
      //cout << "Contact-HB"<<"\t"<< D->GetResName() <<"\t"<<D->GetSeqNum ()<<"\t"<<D->name<<"\t"<< A->GetResName() <<"\t"<<A->GetSeqNum()<<"\t"<<A->name<<"\t"<<Contact[i].dist<< "\n";
      //}   
      
        
      
      //GDL:check other bonds (between donor and acceptor candidates)
      //if ((Contact[i].dist<=maxSBdist)&& (!isSBridge)&& (!isHBond)&&(D->GetResidue()!=A->GetResidue())){

      //   cout << "Contact-vW"<<"\t"<< D->GetResName() <<"\t"<<D->GetSeqNum ()<<"\t"<<D->element<<"\t"<< A->GetResName() <<"\t"<<A->GetSeqNum()<<"\t"<<A->element<<"\t"<<Contact[i].dist<< "\n";
      //}
  ...
  /*CCP4SRS_RC Chem::CalcOtherContacts ( mmdb::PPResidue Res1, int nres1,
                                mmdb::PPResidue Res2, int nres2,
                                RPAtomPair     OthBond, int & nOthBonds,
                                mmdb::io::PFile structFile,
                                mmdb::pstr altLoc,
                                bool ignoreNegSigOcc )  {
  mmdb::io::PFile sFile;
  ...
  mmdb::pstr      resName;
  int             nDonors1,nAcceptors1, nDonors2,nAcceptors2, nContacts;
  int             i,j,k,nDBonds,nABonds,nOthBondAlloc;
  bool            isOthBond;
  int             natom1, natom2;
  mmdb::PPAtom    atom1, atom2;
    ...
    for (i=0;i<nContacts;i++)  {
      ...
      isOthBond = false ; // GDL added this 
      ...
      if ((Contact[i].dist<=maxSBdist)       &&
        ...
        if (!strcmp(D->name," N  "))  {
          // mainchain nitrogen can form salt bridge only at N-terminus
          isOthBond = D->isNTerminus();
        } else  {
          // other nitrogens can form salt bridge only in LYS, ARG
          // and HIS
          resName   = D->GetResName();
          isOthBond = ((!strcmp(resName,"LYS")) ||
                       (!strcmp(resName,"ARG")) ||
                       (!strcmp(resName,"HIS")));
        }
        if (isOthBond)  {
          if ((!strcmp(A->name," O  ")) || (!strcmp(A->name," OXT")))  {
            // mainchain oxygens can form salt bridge only at C-terminus
            isOthBond = A->isCTerminus();
          } else  {
            // other oxygens can form salt bridge only in GLU and ASP
            resName   = A->GetResName();
            isOthBond = ((!strcmp(resName,"GLU")) ||
                         (!strcmp(resName,"ASP")));
          }
          if (isOthBond)  {
            if (Contact[i].group<=1)
                  _addAtomPair ( D,A,OthBond,nOthBonds,nOthBondAlloc );
            else  _addAtomPair ( A,D,OthBond,nOthBonds,nOthBondAlloc );
          }
	}
